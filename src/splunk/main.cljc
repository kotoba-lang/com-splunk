(ns splunk.main
  "Kotodama WASM entrypoint for the Splunk clean-room actor (L4) — Clojure port.
  ...
  facts (`splunk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "splunk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"splunk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "splunk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :splunk-compat/ready)
